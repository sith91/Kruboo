...
def open_application(app_name: str) -> str:
    ...
    if _is_android():
        try:
            ...
            pkg = app_packages.get(app_lower)
            if not pkg:
                # Find matching package by label
                for p in pm.getInstalledPackages(0):
                    label = p.applicationInfo.loadLabel(pm).toString().lower()
                    if app_lower in label:
                        pkg = p.packageName
                        break
            if pkg:
                ...
                if intent:
                    ...
                    return f"Opening {app_name} on your Android device."
            return f"Could not find application '{app_name}' installed on your Android device."
        except Exception as e:
            return f"Failed to open {app_name} on Android. Error: {e}"
...
def manage_files(action: str, filepath: str, content: str = None) -> str:
    ...
    try:
        ...
        allowed_dir = memory.get_setting("allowed_workspace_dir")
        
        if allowed_dir:
            workspace_dir = os.path.abspath(allowed_dir)
        else:
        ...
        if action == "read":
            ...
            return f"Found matches: {', '.join(matches) if matches else 'No matches found'}"
            ...
            return f"Opened file '{filepath}' on your computer."
...
def make_call(phone_number: str) -> str:
    """Makes a call on the local system or opens dialer on mobile."""
    if _is_android():
        try:
            from com.chaquo.python import Python
            from android.content import Intent
            ...
            intent.setData(Uri.parse(f"tel:{clean_num}"))
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            context.startActivity(intent)
            return f"Opening dialer to call {phone_number} on mobile."
        except Exception as e:
            return f"Failed to make call. Error: {e}"
    else:
        if platform.system() == "Darwin":
            subprocess.Popen(["open", f"tel:{phone_number}"])
            return f"Opening FaceTime to call {phone_number} on Mac."
        return "Calling is not supported on this platform."

def set_alarm(time_str: str, message: str = "Kruboo Alarm") -> str:
    """Sets an alarm on the mobile system or simulates on desktop. E.g. '07:30'."""
    try:
        parts = time_str.split(":")
        hour = int(parts[0])
        minute = int(parts[1])
    except:
        return f"Invalid time format '{time_str}'. Please use HH:MM format."
    ...
    if _is_android():
        try:
            ...
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            context.startActivity(intent)
            return f"Alarm set successfully for {hour:02d}:{minute:02d}."
        except Exception as e:
            return f"Failed to set alarm. Error: {e}"
    else:
        # Simple simulated timer/alarm scheduled via desktop background thread
        import threading
        import time
        ...
        def run_alarm():
            ...
            if platform.system() == "Darwin":
                subprocess.run(["osascript", "-e", f'display notification "{message}" with title "Kruboo Alarm" sound name "Glass"'])
        threading.Thread(target=run_alarm, daemon=True).start()
        return f"Alarm scheduled for {hour:02d}:{minute:02d} on desktop."

def read_sms_messages(limit: int = 5) -> str:
    """Reads recent SMS messages on Android."""
    if _is_android():
        try:
            from com.chaquo.python import Python
            from android.net import Uri
            ...
            cursor = context.getContentResolver().query(
                ...
                ["address", "body", "date"],
                None, None, "date DESC LIMIT " + str(limit)
            )
            messages = []
            if cursor:
                while cursor.moveToNext():
                    ...
                    messages.append(f"From {address}: {body}")
                cursor.close()
            if not messages:
                return "No recent SMS messages found."
            return "Recent messages:\n" + "\n".join(messages)
        except Exception as e:
            return f"Could not read messages. Please verify SMS permissions are granted. Error: {e}"
    else:
        return "Reading local system messages is not supported on this platform."
