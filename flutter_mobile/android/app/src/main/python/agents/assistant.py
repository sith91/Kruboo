...
def handle_user_query(
    ...
    if action_intent:
        if action_intent == "open_app":
            ...
            if _gmail_plugin:
                ...
                tool_result = "Gmail integration is not available on mobile."
            ...
            if _calendar_plugin:
                ...
                tool_result = "Calendar integration is not available on mobile."
            action = "get_calendar"
            ...
            start_time = dt_module.datetime.utcnow().isoformat() + 'Z' 
            end_time = (dt_module.datetime.utcnow() + dt_module.timedelta(hours=1)).isoformat() + 'Z'
            if _calendar_plugin:
    ...
    short_circuit_actions = ["open_app", "close_app", "save_note", "get_time", "get_weather", "get_system_stats", "media_", "save_memory", "iot_", "file_", "make_call", "set_alarm", "read_sms"]
...
async def stream_user_query(
    ...
    # Short-circuit for system-level actions that don't need LLM synthesis
    short_circuit_actions = ["open_app", "close_app", "save_note", "get_time", "get_weather", "get_system_stats", "media_", "new_chat", "save_memory", "file_", "make_call", "set_alarm", "read_sms"]
    if any(action.startswith(prefix) for prefix in short_circuit_actions):
        memory.add_message(chat_id, "user", query)
        memory.add_message(chat_id, "assistant", str(tool_result))
